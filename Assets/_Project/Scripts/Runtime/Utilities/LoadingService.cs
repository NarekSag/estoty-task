...
namespace Scripts.Runtime.Utilities
{
    ...
    public sealed class LoadingService
    {
        ...
        {
            _watch?.Stop();
            Debug.Log($"{unit.GetType().Name} is {(isError ? "NOT " : "")}loaded with time {_watch?.ElapsedMilliseconds}ms");
            ...
            {
                _watch?.Restart();
                Debug.Log($"THREADING: start switching from '{currentThreadId}' thread to main thread '{mainThreadId}'");
                await UniTask.SwitchToMainThread();
                _watch?.Stop();
                Debug.Log($"THREADING: switch finished with time {_watch?.ElapsedMilliseconds}");
            }
        }
